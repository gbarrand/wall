...
#define ASSERT(a_what,a_where) if(!(a_what)) {printf("debug : Contour : assert failure in %s\n",a_where);::exit(0);} //G.Barrand

#ifdef _DEBUG
#undef THIS_FILE
static char THIS_FILE[]=__FILE__;
//GB #define new DEBUG_NEW
#endif 

double ContourTestFunction(double x,double y,void*)
{  
...
//G.Barrand : to pass on Windows std::max -> inlib::mx :
namespace in_lib {
template <class T>
...
void CContour::SetFieldFcn(double (*_pFieldFcn)(double, double,void*),void* aData) 
{	
...
{
        m_iColFir=in_lib::mx<int>(iCol,2);
        m_iRowFir=in_lib::mx<int>(iRow,2);
}
...
void CContour::SetSecondaryGrid(int iCol, int iRow)
{
	...
	m_iColSec=in_lib::mx<int>(iCol,2);
	m_iRowSec=in_lib::mx<int>(iRow,2);
}
...
double CContour::GetPlane(unsigned int i)	const	{	/*ASSERT(i>=0);*/ ASSERT(i<m_vPlanes.size(),"CContour::GetPlane"); return m_vPlanes[i];}
double CContour::GetYi(int i) const { if(i<0) printf("CContour::GetYi : %d\n",i);ASSERT(i>=0,"CContour::GetYi"); return m_pLimits[2] +  i/(m_iColSec+1)*(m_pLimits[3]-m_pLimits[2])/(double)( m_iRowSec );}
