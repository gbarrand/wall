...
typedef short INT16;
typedef long  _INT32;  //G.Barrand : _ for Windows.
typedef unsigned char UINT8;
typedef unsigned short UINT16;
typedef int _boolean;
#define GET_JSAMPLE(value)  (value)
...
static struct {
 ...
 _boolean    quantize_colors;
} _Class = {NULL,0,0,CS_RGB,8,TRUE};
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  _Class.output_file     = (FILE*)a_file;
  _Class.image_width     = a_width;
  _Class.image_height    = a_height;
   ...
   */
  if (_Class.quantize_colors==FALSE) emit_header(256, (JSAMPARRAY) NULL);
}
//////////////////////////////////////////////////////////////////////////////
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  ...
  putc(0, _Class.output_file);
  /* Write the GIF terminator mark */
  putc(';', _Class.output_file);
  /* Make sure we wrote the output file OK */
  fflush(_Class.output_file);
  if (ferror(_Class.output_file)!=0) 
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  if (bytesinpkt > 0) {                /* never write zero-length packet */
    ...
    if (JFWRITE(_Class.output_file, packetbuf, bytesinpkt) 
        != (size_t) bytesinpkt) 
...
static _INT32 cur_accum;                /* holds bits not yet output */
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  cur_accum |= ((_INT32) code) << cur_bits;
  cur_bits += n_bits;
...
static _boolean  first_byte;        /* if TRUE, waiting_code is not valid */
//////////////////////////////////////////////////////////////////////////////
void clear_hash(
)
//////////////////////////////////////////////////////////////////////////////
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  putc(w & 0xFF, _Class.output_file);
  putc((w >> 8) & 0xFF,_Class.output_file);
}
//////////////////////////////////////////////////////////////////////////////
void put_3bytes(
 int val
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
{
  ...
  int cshift = _Class.data_precision - 8;
  int i;
   ...
   */
  putc('G', _Class.output_file);
  putc('I', _Class.output_file);
  putc('F', _Class.output_file);
  putc('8', _Class.output_file);
  putc('7', _Class.output_file);
  putc('a', _Class.output_file);
  /* Write the Logical Screen Descriptor */
  put_word((UINT16) _Class.image_width);
  put_word((UINT16) _Class.image_height);
  FlagByte = 0x80;                /* Yes, there is a global color table */
  FlagByte |= (BitsPerPixel-1) << 4; /* color resolution */
  FlagByte |= (BitsPerPixel-1);        /* size of global color table */
  putc(FlagByte, _Class.output_file);
  putc(0, _Class.output_file);        /* Background color index */
  putc(0, _Class.output_file);        /* Reserved in GIF87 (aspect ratio in GIF89) */
  ...
  for (i=0; i < ColorMapSize; i++) {
    if (i < num_colors) {
      if (colormap != NULL) {
        if (_Class.out_color_space == CS_RGB) {
          /* Normal case: RGB color map */
          putc(GET_JSAMPLE(colormap[0][i]) >> cshift, _Class.output_file);
          putc(GET_JSAMPLE(colormap[1][i]) >> cshift, _Class.output_file);
          putc(GET_JSAMPLE(colormap[2][i]) >> cshift, _Class.output_file);
        } else {
          /* Grayscale "color map": possible if quantizing grayscale image */
          put_3bytes(GET_JSAMPLE(colormap[0][i]) >> cshift);
        }
      } else {
  ...
  putc(',', _Class.output_file); /* separator */
  put_word((UINT16) 0);                /* left/top offset */
  put_word((UINT16) 0);
  put_word((UINT16) _Class.image_width); /* image size */
  put_word((UINT16) _Class.image_height);
  /* flag byte: not interlaced, no local color map */
  putc(0x00, _Class.output_file);
  /* Write Initial Code Size byte */
  putc(InitCodeSize, _Class.output_file);
