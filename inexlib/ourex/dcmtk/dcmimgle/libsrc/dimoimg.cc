                                     ...
                                     const int padding)
{
    ...
    if ((bits == 8) || (bits == 24) || (bits == 32))
    {
        ...
        if ((OutputData != NULL) && (OutputData->getData() != NULL))
        {
            ...
            /*G.Barrand register*/ const Uint8 *p = OFstatic_cast(const Uint8 *, OutputData->getData()) + ((upsideDown) ?
            ...
            if (bits == 8)                                  // -- for idx color model (byte)
            {
                ...
                if ((gap > 0) || (nextRow != 0) || (data != NULL))
                {
                    if ((data == NULL) || (size >= count))
                    {
                        ...
                        if (data != NULL)
                        {
                            /*G.Barrand register*/ Uint8 *q = OFstatic_cast(Uint8 *, data);
            ...
            else if (bits == 24)                            // -- for direct color model (24 bits/pixel)
            {
                ...
                if ((data == NULL) || (size >= count))
                {
                    ...
                    {
                        /*G.Barrand register*/ Uint8 *q = OFstatic_cast(Uint8 *, data);
                        /*G.Barrand register*/ Uint8 value;
                        /*G.Barrand register*/ Uint16 x;
                        /*G.Barrand register*/ Uint16 y;
                        /*G.Barrand register*/ int j;
                        for (y = Rows; y != 0; --y)
                        {
                            for (x = Columns; x != 0; --x)
                            {
            ...
            else if (bits == 32)                            // -- for direct color model (32 bits/pixel)
            {
                ...
                if ((data == NULL) || (size >= count * 4))
                {
                    ...
                    if (data != NULL)
                    {
                        /*G.Barrand register*/ Uint32 *q = OFstatic_cast(Uint32 *, data);
                        /*G.Barrand register*/ Uint32 value;
                        /*G.Barrand register*/ Uint16 x;
                        /*G.Barrand register*/ Uint16 y;
                        for (y = Rows; y != 0; --y)
                        {
                                           ...
                                           const int bits)
{
    ...
    else if (bits == 32)                                // for direct color model (long int)
    {
        ...
        if ((OutputData != NULL) && (OutputData->getData() != NULL))
        {
            ...
            if (data != NULL)
            {
                /*G.Barrand register*/ const Uint8 *p = OFstatic_cast(const Uint8 *, OutputData->getData());
                /*G.Barrand register*/ Uint32 *q = OFstatic_cast(Uint32 *, data);
                /*G.Barrand register*/ Uint32 value;
...
//          # old code: if ((size * 8 + alloc - 1) / alloc == count)
            if (8 * size_1 + (8 * size_2 + alloc - 1) / alloc == count)
            {
                ...
                if (data != NULL)
                {
                    /*G.Barrand register*/ const Uint16 *p = OFstatic_cast(const Uint16 *, buffer);
                    /*G.Barrand register*/ Uint16 *q = data;
                    /*G.Barrand register*/ unsigned long i;
                    /*G.Barrand register*/ Uint16 value1;
                    /*G.Barrand register*/ Uint16 value2;
