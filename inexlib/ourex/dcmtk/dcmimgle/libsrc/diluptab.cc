    ...
    if (count > 0)                                                            // valid LUT
    {
        ...
        if (count != Count)                                                   // length of LUT differs from number of LUT entries
        {
            if (count == ((Count + 1) >> 1))                                  // bits allocated 8, ignore padding
            {
                ...
                {
                    /*G.Barrand register*/ const Uint8 *p = OFreinterpret_cast(const Uint8 *, Data);
                    /*G.Barrand register*/ Uint16 *q = DataBuffer;
        ...
        /*G.Barrand register*/ const Uint16 *p = Data;
        /*G.Barrand register*/ Uint16 value;
...
int DiLookupTable::invertTable(const int flag)
{
    ...
    if ((Data != NULL) && (Count > 0) && (flag & 0x3))
    {
        ...
        if (flag & 0x2)
        {
            if (OriginalData != NULL)
            {
                if (OriginalBitsAllocated == 8)
                {
                    if (Bits <= 8)
                    {
                        /*G.Barrand register*/ const Uint8 *p = OFconst_cast(const Uint8 *, OFstatic_cast(Uint8 *, OriginalData));
                        /*G.Barrand register*/ Uint8 *q = OFstatic_cast(Uint8 *, OriginalData);
                    ...
                    /*G.Barrand register*/ const Uint16 *p = OFconst_cast(const Uint16 *, OFstatic_cast(Uint16 *, OriginalData));
                    /*G.Barrand register*/ Uint16 *q = OFstatic_cast(Uint16 *, OriginalData);
        ...
        if (flag & 0x1)
        {
            if (DataBuffer != NULL)
            {
                /*G.Barrand register*/ const Uint16 *p = OFconst_cast(const Uint16 *, DataBuffer);
                /*G.Barrand register*/ Uint16 *q = DataBuffer;
                const Uint16 max = OFstatic_cast(Uint16, DicomImageClass::maxval(Bits));
                for (i = Count; i != 0; --i)
                    *(q++) = max - *(p++);
...
int DiLookupTable::mirrorTable(const int flag)
{
    ...
    if ((Data != NULL) && (Count > 0) && (flag & 0x3))
    {
        ...
        if (flag & 0x2)
        {
            if (OriginalData != NULL)
            {
                if (OriginalBitsAllocated == 8)
                {
                    if (Bits <= 8)
                    {
                        /*G.Barrand register*/ Uint8 *p = OFstatic_cast(Uint8 *, OriginalData) + (Count - 1);
                        /*G.Barrand register*/ Uint8 *q = OFstatic_cast(Uint8 *, OriginalData);
                        /*G.Barrand register*/ Uint8 val;
                    ...
                    /*G.Barrand register*/ Uint16 *p = OFstatic_cast(Uint16 *, OriginalData) + (Count - 1);
                    /*G.Barrand register*/ Uint16 *q = OFstatic_cast(Uint16 *, OriginalData);
                    /*G.Barrand register*/ Uint16 val;
                    const unsigned long mid = Count / 2;
                    for (i = mid; i != 0; --i)
                    {
                        val = *q;
        ...
        if (flag & 0x1)
        {
            if (DataBuffer != NULL)
            {
                /*G.Barrand register*/ Uint16 *p = DataBuffer + (Count - 1);
                /*G.Barrand register*/ Uint16 *q = DataBuffer;
                /*G.Barrand register*/ Uint16 val;
                const unsigned long mid = Count / 2;
                for (i = mid; i != 0; --i)
            ...
            else if (!(flag & 0x2))
            {
                ...
                if (DataBuffer != NULL)
                {
                    /*G.Barrand register*/ Uint16 *p = OFconst_cast(Uint16 *, Data) + (Count - 1);
                    /*G.Barrand register*/ Uint16 *q = DataBuffer;
                    /*G.Barrand register*/ Uint16 val;
                    const unsigned long mid = Count / 2;
                    for (i = mid; i != 0; --i)
    ...
    if (Valid)
    {
        ...
        if ((data != NULL) && (valid != NULL))
        {
            ...
            while (i < count)                                   // fill gaps with valid values
            {
                ...
                {
                    /*G.Barrand register*/ Uint32 j = i + 1;
