        ...
        if (Data != NULL)
        {
            ...
            if ((sizeof(T2) <= 2) && (Count > 3 * ocnt))               // optimization criteria
            {
                ...
                if (lut != NULL)
                {
                    ...
                    /*G.Barrand register*/ Uint8 *q = lut - OFstatic_cast(T2, getAbsMinimum());
...
//          # old code: Count = ((lengthBytes * 8) + bitsAllocated - 1) / bitsAllocated;
            ...
            if (Data != NULL)
            {
                ...
                else if ((bitsof_T1 > bitsAllocated) && (bitsof_T1 % bitsAllocated == 0))   // case 2: divisor of 8/16 bit
                {
                    ...
                    for (i = 0; i < bitsStored; ++i)
                        mask |= OFstatic_cast(T1, 1 << i);
                    /*G.Barrand register*/ Uint16 j;
                    /*G.Barrand register*/ T1 value;
                    ...
                    && (bitsStored == bitsAllocated))
                {
                    ...
                    /*G.Barrand register*/ Uint16 j;
                    /*G.Barrand register*/ Uint16 shift;
                    /*G.Barrand register*/ T2 value;
                    for (i = length_T1; i != 0; --i)
                    {
                        shift = 0;
                ...
                else                                                                        // case 4: anything else
                {
                    ...
                    /*G.Barrand register*/ Uint16 bits = 0;
                    /*G.Barrand register*/ Uint32 skip = highBit + 1 - bitsStored;
                    /*G.Barrand register*/ Uint32 times;
                    T1 mask[bitsof_T1];
                    mask[0] = 1;
