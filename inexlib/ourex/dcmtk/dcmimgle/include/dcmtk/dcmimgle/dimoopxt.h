    ...
    inline void determineUsedValues()
    {
        if ((UsedValues == NULL) && (MaxValue > 0) && (MaxValue < MAX_TABLE_ENTRY_COUNT))
        {
            ...
            if (UsedValues != NULL)
            {
                ...
                /*G.Barrand register*/ const T3 *p = Data;
                /*G.Barrand register*/ Uint8 *q = UsedValues;
                /*G.Barrand register*/ unsigned long i;
                for (i = Count; i != 0; --i)
        ...
        if ((pixel != NULL) && (vlut != NULL))
        {
            ...
            if (Data != NULL)
            {
                ...
                if (minvalue == vlut->getMaxValue())                                    // LUT has only one entry or all entries are equal
                {
                    ...
                    /*G.Barrand register*/ const T1 *p = pixel + start;
                    /*G.Barrand register*/ T3 *q = Data;
                    T3 *lut = NULL;
                    if ((plut != NULL) && (plut->isValid()))                            // has presentation LUT
                    {
                        DCMIMGLE_DEBUG("applying presentation LUT transformation");
                        createDisplayLUT(dlut, disp, plut->getBits());
                        /*G.Barrand register*/ Uint32 value2;                                         // presentation LUT is always unsigned
        ...
        if (pixel != NULL)
        {
            ...
            if (Data != NULL)
            {
                ...
                if ((plut != NULL) && (plut->isValid()))                              // has presentation LUT
                {
                    DCMIMGLE_DEBUG("applying presentation LUT transformation");
                    createDisplayLUT(dlut, disp, plut->getBits());
                    /*G.Barrand register*/ Uint32 value;                                            // presentation LUT is always unsigned
                    ...
                    /*G.Barrand register*/ const double gradient = outrange / (inter->getAbsMaxRange());
                    if (initOptimizationLUT(lut, ocnt))
                    {                                                                 // use LUT for optimization
                        q = lut;
                        if (dlut != NULL)                                             // perform display transformation
                 ...
                 const T3 high)
    {
        ...
        if (pixel != NULL)
        {
            ...
            if (Data != NULL)
            {
                ...
                /*G.Barrand register*/ T3 *q = Data;
                /*G.Barrand register*/ unsigned long i;
                /*G.Barrand register*/ double value;
                T3 *lut = NULL;
                if ((plut != NULL) && (plut->isValid()))                              // has presentation LUT
                {
                    DCMIMGLE_DEBUG("applying presentation LUT transformation");
                        ...
                        if ((plane != NULL) && plane->isVisible() && plane->reset(frame))
                        {
                            ...
                            switch (plane->getMode())
                            {
                                ...
                                case EMO_BitmapShutter:
                                {
                                    ...
                                    /*G.Barrand register*/ T3 fore = OFstatic_cast(T3, OFstatic_cast(double, maxvalue) * OFstatic_cast(double, plane->getPValue()) / OFstatic_cast(double, DicomImageClass::maxval(WIDTH_OF_PVALUES)));
                                    if ((disp != NULL) && (disp->isValid()))
                                    {
