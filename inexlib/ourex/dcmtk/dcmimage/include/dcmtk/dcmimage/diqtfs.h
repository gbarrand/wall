...
public:
  ...
  {
    /*G.Barrand register*/ long sr = px.getRed()   + thisrerr[col + 1] / DcmQuantFloydSteinbergScale;
    /*G.Barrand register*/ long sg = px.getGreen() + thisgerr[col + 1] / DcmQuantFloydSteinbergScale;
    /*G.Barrand register*/ long sb = px.getBlue()  + thisberr[col + 1] / DcmQuantFloydSteinbergScale;
    if ( sr < 0 ) sr = 0;
