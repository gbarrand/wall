     ...
     */
{
    ...
    /*G.Barrand register*/ Uint8 save;

    /* in case valWidth equals 2, swap correspondingly */
    if (valWidth == 2)
    {
	/*G.Barrand register*/ Uint8 * first = &OFstatic_cast(Uint8*, value)[0];
	/*G.Barrand register*/ Uint8 * second = &OFstatic_cast(Uint8*, value)[1];
	/*G.Barrand register*/ Uint32 times = byteLength/2;
	while(times--)
	{
	    save = *first;
	    *first = *second;
    ...
    {
	/*G.Barrand register*/ size_t i;
	const size_t halfWidth = valWidth/2;
	const size_t offset = valWidth-1;
	/*G.Barrand register*/ Uint8 *start;
	/*G.Barrand register*/ Uint8 *end;
