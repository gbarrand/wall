...
mdsModel(vtx_data * graph, int nG)
{
    ...
    float *Dij;
    int shift = 0;
    double delta = 0; /*G.Barrand : WIN32 : = 0 */
	...
	for (e = 1; e < graph[i].nedges; e++) {
            j = graph[i].edges[e];
            if (j < i) continue;
          /*delta += abs(Dij[i*nG + j - shift] - graph[i].ewgts[e]);*/ /*G.Barrand*/
            delta += fabs(Dij[i*nG + j - shift] - graph[i].ewgts[e]);
...
/*G.Barrand : #define max(x,y) ((x)>(y)?(x):(y))*/
...
float *compute_apsp_artifical_weights_packed(vtx_data * graph, int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		weights[j] =
		    (float)
		  MAX((float)    /*G.Barrand : WIN32 : max -> MAX */
			(deg_i + deg_j -
