 ...
 */
     ...
     */
  /*if (adjust && !haveBackground && (ROUND(abs(GD_bb(g).LL.x)) || ROUND(abs(GD_bb(g).LL.y)))) *//*G.Barrand*/
    if (adjust && !haveBackground && (ROUND(fabs(GD_bb(g).LL.x)) || ROUND(fabs(GD_bb(g).LL.y))))
...
//	fprintf(stderr, "%i\n", count_nodes(g));
    ...
    if (mode != MODE_MAJOR) {
	    ...
	    } else if (str && !gv_strncasecmp(str,"hier",4)) {
                opt.diredges = 2;
                if(Verbose)
