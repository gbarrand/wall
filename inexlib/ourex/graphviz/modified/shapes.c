...
static void poly_init(node_t * n)
{
  ...
  /*if (ROUND(abs(dimen.x)) || ROUND(abs(dimen.y))) { */ /*G.Barrand.*/
    if (ROUND(fabs(dimen.x)) || ROUND(fabs(dimen.y))) { /*G.Barrand.*/
	/* padding */
	if ((p = agget(n, "margin"))) {
...
static field_t *parse_reclbl(node_t * n, int LR, int flag, char *text)
{
	...
	case '>':
		...
		psp--;
	    *psp = '\000';
	    if(tmpport) free(tmpport); /*G.Barrand*/
	    tmpport = strdup(text);
	    mode &= ~INPORT;
	    reclblp++;
	    break;
	...
	case '\000':
		...
		*tsp = '\000';
		fp->lp =
		  /*G.Barrand : make_label((void *) n, strdup(text), */
		  make_label((void *) n, text, /*G.Barrand : no strdup*/
