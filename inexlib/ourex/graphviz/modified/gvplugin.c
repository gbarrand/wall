...
#ifndef GV_HAVE_CONFIG_H /*G.Barrand*/
#include "gv_config.h"
#endif
...
extern int gv_strcasecmp(const char *s1, const char *s2);
#endif
...
{
  /*if (api < 0 || api >= ARRAY_SIZE(api_names)) */ /*G.Barrand*/
...
#define TYPSIZ 63
  ...
  /*if (api < 0) return FALSE;*/ /*G.Barrand : rm warn.*/
		 ...
		 gvplugin_installed_t * typeptr)
{
  ...
  /*if (api < 0) return FALSE;*/ /*G.Barrand : rm warn.*/
    ...
    while (*pnext) {
	if ( (gv_strcasecmp(typestr, (*pnext)->typestr) == 0)
	  && (gv_strcasecmp(name, (*pnext)->package->name) == 0)
	  && ((*pnext)->package->path != 0)
	  && (gv_strcasecmp(path, (*pnext)->package->path) == 0)) {
...
#define TYPBUFSIZ 64
  ...
  /*if (api < 0) return NULL;*/ /*G.Barrand : rm warn.*/
  ...
  /*if (api < 0) return NULL;*/ /*G.Barrand : rm warn.*/
	...
	*p++ = '\0';
	...
	for (pnext = plugin; *pnext; pnext = &((*pnext)->next)) {
	    ...
	    if ((p = strchr(q, ':')))
		*p++ = '\0';
	    if (!typestr_last || gv_strcasecmp(typestr_last, q) != 0) {
...
char **gvPluginList(GVC_t * gvc, char* kind, int* sz, const char *str)
{
    ...
    if (!kind) return NULL;
    for (api = 0; api < ARRAY_SIZE(api_names); api++) {
	if (!gv_strcasecmp(kind,api_names[api]))
	    break;
    }
