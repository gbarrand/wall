...
/*G.Barrand : prefix with gzmemio_ */
typedef void * (*gzmemio_alloc_func)(void * opaque, unsigned int items, unsigned int size);
typedef void   (*gzmemio_free_func)(void * opaque, void * address);
...
typedef struct {
  ...
  gzmemio_alloc_func zalloc;  /* used to allocate the internal state */
  gzmemio_free_func  zfree;   /* used to free the internal state */
...
void * cc_gzm_open(const uint8_t * buffer, uint32_t len)
{
  ...
  s->stream.zalloc = (gzmemio_alloc_func)0;
  s->stream.zfree = (gzmemio_free_func)0;
